...
```
inflation-forecast/
├── data/               # Raw + cleaned CSVs from FRED (gitignored, regenerated by fetch_data.py)
...
├── outputs/            # All saved plot PNGs and CSVs
├── dashboard/          # Streamlit app
│   └── app.py          # 4-tab dashboard (time series, forecasts, comparison, about)
├── paper/
│   └── paper.md        # Full research paper (all sections complete)
├── .streamlit/
│   ├── config.toml     # Theme config
│   └── secrets.toml.example  # Secret key template (real file is gitignored)
├── docs/
...
- [x] dashboard deployed at https://inflation-forecast-vibe.streamlit.app/
- [x] paper/paper.md: full paper written and critique-addressed (Granger tests, random walk, CUSUM, figures, footnotes, Phillips curve, rolling window justification)
- [x] src/paper_analysis.py: reproduces all paper statistics and generates /figures

### In Progress
- [ ] None — project substantially complete

### Next Up
- [ ] Optional: rolling window evaluation
- [ ] Optional: Bayesian VAR extension
